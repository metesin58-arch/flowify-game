...
export const HexagonGame: React.FC<Props> = ({ onExit, onGameEnd }) => {
  ...
  if (gameState === 'gameover') {
      return (
          <div className="h-full w-full bg-black flex flex-col items-center justify-center relative z-50 text-center animate-zoom-in">
              <div className="text-[#ef4444] text-6xl mb-6 animate-pulse">💀</div>
              <h2 className="text-4xl font-black text-white italic tracking-tighter mb-2">ÇAKILDIN</h2>
              <p className="text-neutral-500 text-xs uppercase tracking-widest mb-10">Ritim seni yendi</p>
              
              <div className="bg-[#121212] border border-white/10 p-6 rounded-2xl mb-8 w-64 shadow-2xl">
                  <div className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest mb-2">SKORUN</div>
                  <div className="text-5xl font-black text-[#1ed760] font-mono">{Math.floor(score)}</div>
              </div>

              <div className="flex gap-4">
                  <button onClick={startGame} className="bg-white text-black font-black py-3 px-8 rounded-full uppercase tracking-widest text-xs hover:scale-105 transition-all">
                      TEKRAR
                  </button>
                  <button onClick={() => { onGameEnd(Math.floor(score)); onExit(); }} className="bg-transparent text-neutral-400 border border-neutral-600 font-bold py-3 px-8 rounded-full uppercase tracking-widest text-xs hover:text-white hover:border-white transition-all">
                      ÇIKIŞ
                  </button>
              </div>
          </div>
